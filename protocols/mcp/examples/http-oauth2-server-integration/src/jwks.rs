        ...
        match generate_test_token(
            ...
            Ok(token) => {
                ...
                tokens.insert(key, json!({
                    ...
                    "curl_test": format!("curl -H 'Authorization: Bearer {}' http://localhost:3001/mcp -X POST -H 'Content-Type: application/json' -d '{{\"jsonrpc\":\"2.0\",\"id\":\"test\",\"method\":\"initialize\",\"params\":{{\"protocolVersion\":\"2025-06-18\",\"capabilities\":{{}},\"clientInfo\":{{\"name\":\"OAuth2-Test\",\"version\":\"1.0.0\"}}}}}}'", token)
                }));
            }
            Err(e) => {
